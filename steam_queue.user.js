...
function sublow(id, tp) {
    GM_xmlhttpRequest({
        method: "GET",
        url: `https://steamdb.info/api/GetPriceHistory/?${tp}id=${id}&cc=cn`,
        headers: {
            "Accept": "application/json, text/javascript, */*",
            "Referer": `https://steamdb.info/${tp}/${id}/`,
        ...
        onload: function(response) {
            ...
            if (data && data.success){
                if (data.data.history.length > 0){
                    ...
                    var n = '';
                    $.each(r.n, function(j, item){
                        n += '<div>' + tm(item) + '</div>';
                    });
                    $(`#${id}`).append(`<div>${r.l}(-${r.c}%)</div>${n}`);
                }
            }
        },
        ...
        onload: function(response) {
            if (response.status == 503)
                    ...
                    if (m){
                        $(this).next('div').find('tbody tr').each(function(){
                            ...
                            if ($(d[1]).children('a').length > 0){
                                name =  `<a target=_blank href="https://steamdb.info/bundle/${sub}/">${name}</a>`;
                                sublow(sub, 'bundle');
                            }
                            var t = $(d[4]).attr('data-sort');
                            t = tm(t * 1000);
                            $('#b').append(`<tr><td>${sub}</td><td>${name}</td><td id="${sub}"></td><td>${t}</td></tr>`);
                        });
                    }
                });
                var b = [];
                $(response.responseText).find('.package').each(function(){
                    ...
                    t = tm(t * 1000);
                    $('#b').append(`<tr><td>${sub}</td><td>${name}</td><td id="${sub}"><div>${tp}</div></td><td>${t}</td></tr>`);
                });
            }
        },
        onerror:  function(response) {
